...
class grafo():
    ...
    def encontrar_arista_con_mayor_peso(self):
        max_peso_arista = None  
        arista_max_peso = None  
        for vertice in self.__elements:
            aristas = vertice[1]  
            for index in range(aristas.size()):
                arista = aristas.get_element_by_index(index)
                if max_peso_arista is None or arista.peso > max_peso_arista:
                    max_peso_arista = arista.peso
                    arista_max_peso = arista
        if arista_max_peso is not None:
            print(f"Arista con el mayor peso: {arista_max_peso.vertice} - Peso: {arista_max_peso.peso}")
        else:
    ...
    def kruskal(self):
        ...
        for index in range(self.size()):
            ...
            for i in range(aristas_adjacentes.size()):
                ...
                arista.arrive([vertice[0], arista.vertice], arista.peso)

        while len(bosque) > 1 and arista.size() > 0:
            arista = arista.atention()
